...
Actualmente, existen soluciones en el mercado como _Square_, _GloriaFood_ o los propios TPVs bancarios. Sin embargo, estas presentan barreras para el hostelero medio:
...
- **RF-06 — Tablero Kanban en Tiempo Real:**
    - Columnas: _Pendiente_, _En Preparacion_, _Listo_.
...
- **RF-11 — Dashboard Analítico (BI):**
    - Visualización de ventas por hora y métricas de productos estrella mediante gráficos interactivos.
    - Resumen diario de facturación y volumen de pedidos.

- **RF-12 — Gestión de Carta Multi-idioma:**
    - Soporte nativo para nombres y descripciones en Inglés (ES/EN).
    - Editor de traducciones integrado en el panel de administración.

- **RF-13 — Facturación Histórica:**
    - Registro legal de cierres de mesa con detalle de base imponible e IVA.
    - Filtros por fecha para consulta de ventas pasadas.

- **RF-14 — Gestión de QR y Mesas:**
    - Generación de códigos unicos asociados a cada mesa.
    - Control de estado de mesas (Ocupada, Libre, Llamando, Pide Cuenta).
...
- **RNF-03 — Usabilidad:** Diseño _Mobile First_ estricto, cumpliendo estandares de accesibilidad (tamaño de botones para dedos).
...
> _Nota: Ver diagrama Entidad-Relacion en la seccion de Anexos._
...
- **Seguridad:**
    - **Control de Acceso (RBAC)**: Rutas administrativas protegidas por `ROLE_ADMIN`.
    - **Defensa en Profundidad**: Middleware de cabeceras de seguridad (`CSP`, `HSTS`, `X-Frame-Options`).
    - **Audit Logs**: Historial detallado de todas las acciones críticas de gestión (quién, qué, cuándo e IP).
    - **Rate Limiting**: Limitador de intentos de login para prevenir ataques de fuerza bruta.
    - **Sanitización**: Limpieza de inputs y validación estricta de carritos para evitar inyecciones.
- **Accesibilidad:**
...
_(Insertar aqui la imagen exportada de dbdiagram.io)_
