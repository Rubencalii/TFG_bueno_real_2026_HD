    ...
    if (loading && pedidos.length === 0) {
        return (
            <div className="py-20 text-center">
                ...
                <p className="text-text-muted font-bold">Cargando tus pedidos...</p>
            </div>
        );
    }
    ...
    if (pedidos.length === 0) {
        return (
            <div className="py-20 text-center px-6">
                ...
                <h3 className="text-xl font-black text-text-main mb-2">No tienes pedidos activos</h3>
                <p className="text-text-muted">Cuando realices un pedido, aparecerá aquí para que puedas seguir su estado.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 max-w-[600px] mx-auto">
            <div className="flex items-center justify-between mb-8">
                <h2 className="text-2xl font-black text-text-main">Mis Pedidos</h2>
            ...
            {pedidos.map((pedido) => {
                ...
                return (
                    <div key={pedido.id} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                        {/* Status Header */}
                            ...
                            <div className="flex items-center justify-between mb-2">
                                {[1, 2, 3].map((s) => (
                                    <div key={s} className="flex flex-col items-center gap-1">
                                        <div className={`size-2 rounded-full ${status.step >= s ? status.color.replace('text', 'bg') : 'bg-gray-200'}`}></div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="p-6 space-y-3">
                            {pedido.detalles.map((detalle, idx) => (
                                <div key={idx} className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <span className="size-6 bg-gray-50 rounded-lg flex items-center justify-center text-[10px] font-black text-primary border border-gray-100">
                                            {detalle.cantidad}
                                        </span>
                                        <span className="text-sm font-bold text-text-main">{detalle.producto}</span>
                                    </div>
                                    {detalle.notas && (
                                        <span className="text-[10px] text-text-muted italic">"{detalle.notas}"</span>
                                    )}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="px-6 py-4 bg-gray-50/50 border-t border-gray-50 flex justify-between items-center">
                            <span className="text-xs font-bold text-text-muted">Total del pedido</span>
                            <span className="font-black text-text-main">{parseFloat(pedido.total).toFixed(2)}€</span>
                        </div>
                    </div>
                );
            })}
