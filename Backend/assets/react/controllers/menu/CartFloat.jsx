        ...
        <>
                ...
                <div className="fixed inset-0 bg-black/50 z-50" onClick={() => setIsOpen(false)}>
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={() => setIsOpen(false)} className="text-text-muted">
                        ...
                        <div className="space-y-4 mb-6">
                            {items.map((item, index) => (
                                <div key={`${item.id}-${item.notas}-${index}`} className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl">
                                    <div className="flex-1">
                                        <p className="font-bold text-sm">{item.nombre}</p>
                                        {item.notas && (
                                            <p className="text-xs text-text-muted italic">"{item.notas}"</p>
                                        )}
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button 
                                            ...
                                            className="size-8 bg-gray-200 rounded-lg flex items-center justify-center"
                                        >
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-6 z-40 pointer-events-none">
                <div className="max-w-[1200px] mx-auto pointer-events-auto">
                    <div className="bg-white/95 backdrop-blur-2xl shadow-[0_15px_50px_rgba(0,0,0,0.1)] rounded-2xl sm:rounded-3xl border border-gray-100 p-4 sm:p-5 flex flex-wrap items-center justify-between gap-4 sm:gap-6">
                        {/* Cart Info */}
                        <button 
                            onClick={() => setIsOpen(true)}
                            className="flex items-center gap-4 sm:gap-6"
                        >
                            <div className="size-12 sm:size-14 bg-primary/10 rounded-xl sm:rounded-2xl flex items-center justify-center text-primary relative border border-primary/20">
                                <span className="material-symbols-outlined text-2xl sm:text-3xl font-bold">shopping_bag</span>
                                <div className="absolute -top-2 -right-2 size-5 sm:size-6 bg-secondary text-white text-[10px] sm:text-xs font-black rounded-full flex items-center justify-center border-2 border-white">
                                    {count}
                                </div>
                            </div>
                            <div className="text-left">
                                <p className="text-base sm:text-lg font-black text-text-main">
                                    {count} artículo{count !== 1 ? 's' : ''} • <span className="text-primary">{total.toFixed(2)} €</span>
                                </p>
                                <p className="text-[9px] sm:text-[10px] text-text-muted font-bold uppercase tracking-widest">
